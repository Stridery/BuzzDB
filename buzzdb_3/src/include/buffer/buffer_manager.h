...
struct LockRequest {
	...
	LockRequest(uint64_t txn_id_, LockMode mode_)
		: txn_id(txn_id_), mode(mode_), granted(false) {}
...
class LockManager {
	...
	std::unordered_map<uint64_t, PageLock> lock_table;
	std::unordered_map<uint64_t, std::set<uint64_t>> txn_to_pages;
	std::unordered_map<uint64_t, std::set<uint64_t>> waits_for;

	bool detect_deadlock(uint64_t start, std::set<uint64_t>& visited) {
		if (visited.count(start)) return true;
		visited.insert(start);
		for (auto neighbor : waits_for[start]) {
			if (detect_deadlock(neighbor, visited)) return true;
		}
		visited.erase(start);
		return false;
	}
