...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance1) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
    TransactionManager transaction_manager(log_manager, buffer_manager);

    uint64_t table_id = 2025;
    const int num_txns = 2;
    const int keys_per_txn = 100000;
    

	std::vector<uint64_t> txn_ids(num_txns);
std::vector<bool> committed(num_txns, false);

// 1. 所有事务一起开始
for (int i = 0; i < num_txns; ++i) {
    txn_ids[i] = transaction_manager.start_txn();
}

// 2. 交错插入，每轮给每个 txn 插入一条
for (int j = 0; j < keys_per_txn; ++j) {
    for (int i = 0; i < num_txns; ++i) {
        int key = i * 100 + j;
        insert_row(heap_segment, transaction_manager, txn_ids[i], table_id, key);

        // 3. 按策略决定是否 commit 某些事务
        if (!committed[i]) {
            if ((j == 2 && i % 3 == 0) || (j == 3 && i % 4 == 1)) {
                transaction_manager.commit_txn(txn_ids[i]);
                committed[i] = true;
            }
        }
    }
	if(j % 200 == 0) log_manager.log_checkpoint(buffer_manager);
}

// 4. 补 commit 剩下的事务（选部分）
for (int i = 0; i < num_txns; ++i) {
    if (!committed[i] && i % 5 == 0) {
        transaction_manager.commit_txn(txn_ids[i]);
        committed[i] = true;
    }
}

    // flush + simulate crash
    buffer_manager.flush_all_pages();

    // 记录开始恢复时间
    auto start = std::chrono::high_resolution_clock::now();

    crash(transaction_manager, buffer_manager, log_manager); // 包括 recovery

    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = end - start;
    std::cout << "\n🌟 Recovery took " << elapsed.count() << " seconds\n";
	FAIL();
    
}

TEST_F(LogManagerTest, TestParallelRecoveryPerformance2) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance3) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
    ...
    uint64_t table_id = 2025;
    const int num_txns = 50;
    const int keys_per_txn = 4000;
...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance4) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance5) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance6) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance7) {
    BufferManager buffer_manager(12800, 1000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
...
TEST_F(LogManagerTest, TestParallelRecoveryPerformance8) {
    BufferManager buffer_manager(128000, 10000);
    auto logfile = buzzdb::File::open_file(LOG_FILE, buzzdb::File::WRITE);
    LogManager log_manager(logfile.get());
    HeapSegment heap_segment(123, log_manager, buffer_manager);
    TransactionManager transaction_manager(log_manager, buffer_manager);
...
for (int j = 0; j < keys_per_txn; ++j) {
    for (int i = 0; i < num_txns; ++i) {
		if (committed[i]) continue; // 🚨 跳过已提交 txn
        int key = i * 100 + j;
        insert_row(heap_segment, transaction_manager, txn_ids[i], table_id, key);
