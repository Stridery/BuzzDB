...
double JoinOptimizer::estimate_join_cost(
    ...
    double cpu_cost = static_cast<double>(card1) * card2;
...
int JoinOptimizer::estimate_join_cardinality(
    ...
    UNUSED_ATTRIBUTE std::map<std::string, TableStats>& stats) {
        ...
        // Assume equality join:
        if (t1pkey && !t2pkey) {
            return static_cast<int>(card2);
        }
        if (!t1pkey && t2pkey) {
            return static_cast<int>(card1);
        }
        if (t1pkey && t2pkey) {
            return static_cast<int>(std::min(card1, card2));
        }
    
        // If no primary key, fallback heuristic (e.g., 30% of cross product)
...
std::vector<LogicalJoinNode> JoinOptimizer::order_joins(
    ...
    for (size_t i = 1; i <= _joins.size(); ++i) {
        ...
        for (const auto& subset : subsets) {
            ...
            for (const LogicalJoinNode& j : subset) {
                CostCard current_cc;
                bool success = compute_cost_and_card_of_subplan(
                    stats, filter_selectivities, j, subset,
                    best_cc.cost, pc, current_cc);

                if (success && current_cc.cost < best_cc.cost) {
                    best_cc = current_cc;
                }
            }

            if (best_cc.cost < DBL_MAX) {
                std::set<LogicalJoinNode> subset_set = subset;
                pc.add_plan(subset_set, best_cc.cost, best_cc.card, best_cc.plan);
            }
        }
    }

    // 全部 join 完成后的最优顺序
    std::set<LogicalJoinNode> all_join_set(_joins.begin(), _joins.end());
    best_order = pc.get_order(all_join_set);
    return best_order;
}
